...
function parseBinaryTRF(buffer) {
  ...
  const headerInfo = extractTRFHeader(buffer);
  console.log('Parsing binary TRF file:', headerInfo);
  ...
  console.log(`Binary data section starts at offset: ${dataStart}`);
...
function extractTRFHeader(buffer) {
  const text = buffer.toString('utf-8');
  const lines = text.split(/\r?\n/);
  ...
  for (let i = 0; i < Math.min(lines.length, 20); i++) {
    ...
    if (line.includes('Sample Rate') || line.includes('Fs')) {
      const match = line.match(/(\d+)/);
      if (match) {
        header.sampleRate = parseInt(match[1]);
      }
    }
    
    if (line.includes('Points') || line.includes('Count')) {
      const match = line.match(/(\d+)/);
      if (match) {
        header.pointCount = parseInt(match[1]);
      }
    }
  }
...
function analyzeTRFData(points) {
  if (points.length === 0) {
    return null;
  }
...
function assessDataQuality(points, analysis) {
  const assessment = {
    overall: 'good',
    issues: [],
    warnings: [],
...
function apply22kHzCutoff(points) {
...
function detectOptimalSampleRate(points) {
